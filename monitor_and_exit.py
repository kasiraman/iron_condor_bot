"""
Exit-management monitor for the SPY 0DTE iron condor bot (Alpaca paper trading).

This is a companion script to iron_condor_bot.py. It does NOT open positions --
it watches whatever iron_condor_bot.py already opened today and closes it early if:
...
order out for. This mirrors iron_condor_bot.py / settle_trades.py, which are also
short-lived, cron-invoked scripts rather than daemons.

Integration with settle_trades.py: if a position shows up here as "closed" (closed
early, before expiration), settle_trades.py should use the actual entry_credit vs
...
positions that were never closed early. See settle_trades.py's `get_early_close()`.
"""
...
def cost_to_close(quotes, short_put_sym, long_put_sym, short_call_sym, long_call_sym):
    ...
    iron_condor_bot.py's entry credit calc (shorts at bid, longs at ask) with the
...
def get_entry_credit(trade_client, row):
    """Same abs()-based sign fix used in settle_trades.py: this strategy only ever
