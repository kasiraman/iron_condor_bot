...
NOTE: this is the renamed 0dte_ version of what used to be settle_trades.py -- same
code, same logs/trades.csv / logs/trade_outcomes.csv / logs/closed_early.csv (unprefixed,
kept for continuity with existing history), just a clearer name now that a second
strategy (weekend_settle_trades.py, for the Friday-to-Monday hold) exists alongside it.

Run this once after market close (e.g. 4:15pm ET), after 0dte_iron_condor_bot.py has
already run at the open. It:

  1. Reads logs/trades.csv (the entry log written by 0dte_iron_condor_bot.py) for rows
...
load_dotenv()
log = get_logger("0dte_settle_trades")

QTY = int(os.getenv("QTY", "1"))

# IMPORTANT: this must NOT default to "iex" the way 0dte_iron_condor_bot.py's live
# spot-price lookup does. IEX only reflects trades that happened on the IEX exchange --
# a small slice of total volume -- so IEX's "last trade of the day" is often noticeably
# different from the real, official closing price (which comes from the primary
# exchange's closing auction via the consolidated SIP tape). Using IEX here was silently
# feeding a wrong close price into the settlement math, which can make a real partial
# loss look like a full max-loss (or vice versa) even though no error is raised.
#
...
OUTCOMES_CSV = LOG_DIR / "trade_outcomes.csv"  # unprefixed on purpose
...
def get_early_close(order_id):
    """Checks logs/closed_early.csv (written by 0dte_monitor_and_exit.py) for a completed
    ...
    was held to expiration as normal (the common case when 0dte_monitor_and_exit.py
    never triggers)."""
...
def settle_row(trade_client, stock_data_client, row):
    ...
    if early:
        ...
        if abs(early_entry_credit - fill_credit) > 0.01:
            notes.append(
                f"entry_credit recorded by 0dte_monitor_and_exit.py (${early_entry_credit:.2f}) differs "
                f"from fill_credit computed here (${fill_credit:.2f}) -- using the monitor's value since "
                "it's tied directly to the actual closing trade."
            )
            fill_credit = early_entry_credit
        gross_pnl = (fill_credit - exit_debit) * 100 * qty
        notes.append(
            f"Position was closed EARLY by 0dte_monitor_and_exit.py (trigger={early.get('trigger')}, "
    ...
    if fee_error:
        ...
        if len(fills) > 4 and not early:
            notes.append(
                ...
                f"a closing trade instead, or check whether 0dte_monitor_and_exit.py closed it (if so, "
