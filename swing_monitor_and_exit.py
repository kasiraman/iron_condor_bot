"""
Exit-management monitor for the SPY swing (multi-week) iron condor bot (Alpaca
paper/live trading).

Companion to swing_iron_condor_bot.py. Unlike the 0DTE monitor (runs one day) or the
weekend monitor (runs one specific expiration day), this one needs to keep running on
EVERY trading day for the entire life of a swing position, since it can stay open for
weeks. It finds "currently open" positions by querying swing_trades.csv for any
non-dry-run row with an order_id whose expiration_date hasn't passed yet and that isn't
already marked closed in swing_closed_early.csv -- not by matching a specific date, since
there's no single day that's "the" day to check.

Closes the open position early if:
  - profit reaches SWING_PROFIT_TARGET_PCT (default 50%) of the credit collected, or
  - loss reaches SWING_STOP_LOSS_PCT (default 200%) of the credit collected.

These defaults are intentionally different from the 0DTE/weekend monitors' 80%/120%: for
a 45-DTE hold, decay is slow early and accelerates near expiration, so waiting for 80% of
max profit can mean holding for most of the position's life; 50% is a commonly-cited
convention for this DTE range specifically to avoid that. Fully overridable via .env.
...
Definitions (both relative to the credit collected at entry, `entry_credit`):
  ...
  -> close when profit_pct >= 0.50  (cost_to_close has fallen to <= 50% of credit)
  -> close when loss_pct   >= 2.00  (cost_to_close has risen to >= 300% of credit)

SCHEDULING NOTE: because a swing position moves much more slowly than a 0DTE position,
running this every 1-2 minutes (like the 0DTE monitor) is unnecessary API load for
little benefit -- every 15-30 minutes during market hours is plenty. See the README.

DESIGN: like the other monitors, this is meant to be invoked repeatedly, not run as one
long-lived process. Each invocation is stateless except for logs/swing_closed_early.csv,
which is what makes repeated invocations safe (idempotent).

Integration with swing_settle_trades.py: if a position shows up here as "closed" (closed
early, before expiration), swing_settle_trades.py should use the actual entry_credit vs
exit_debit recorded here for realized P&L instead of computing hold-to-expiration
settlement value against the close price. See swing_settle_trades.py's `get_early_close()`.
"""
...
load_dotenv()
log = get_logger("swing_monitor_and_exit")

QTY = int(os.getenv("SWING_QTY", "1"))

OPTION_DATA_FEED = OptionsFeed(os.getenv("OPTION_DATA_FEED", "indicative"))

# Intentionally different defaults from the 0DTE/weekend monitors -- see module docstring.
PROFIT_TARGET_PCT = float(os.getenv("SWING_PROFIT_TARGET_PCT", "0.50"))
STOP_LOSS_PCT = float(os.getenv("SWING_STOP_LOSS_PCT", "2.00"))
PROFIT_CLOSE_BUFFER = float(os.getenv("SWING_PROFIT_CLOSE_BUFFER", "0.02"))
STOP_LOSS_BUFFER = float(os.getenv("SWING_STOP_LOSS_BUFFER", "0.10"))
MONITOR_END_TIME = os.getenv("SWING_MONITOR_END_TIME", "15:55")
PENDING_CLOSE_TIMEOUT_MIN = int(os.getenv("SWING_PENDING_CLOSE_TIMEOUT_MIN", "10"))
RESUBMIT_BUFFER_STEP = float(os.getenv("SWING_RESUBMIT_BUFFER_STEP", "0.05"))
MAX_ESCALATIONS = int(os.getenv("SWING_MAX_ESCALATIONS", "5"))
...
def cost_to_close(quotes, short_put_sym, long_put_sym, short_call_sym, long_call_sym):
    ...
    now: buy back the shorts at their ask, sell the longs at their bid."""
    return (
...
def get_entry_credit(trade_client, row):
    ...
    condor, so the real credit is always positive by construction."""
...
def evaluate_new_position(trade_client, option_data_client, row, past_cutoff, dry_run):
    ...
    log.info(
        f"{row['date']} (expires {row.get('expiration_date', '?')}, target_dte={row.get('target_dte', '?')})  "
        f"order={order_id}  entry_credit={entry_credit:.2f}  cost_to_close={ctc:.2f}  "
        f"profit_pct={profit_pct:.1%}  loss_pct={loss_pct:.1%}"
    ...
    if past_cutoff:
        log.info(f"  (past SWING_MONITOR_END_TIME={MONITOR_END_TIME} -- skipping new close, will re-evaluate next run or let swing_settle_trades.py handle it at expiration)")
        return None
...
# --------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Exit-management monitor for the open swing (multi-week) iron condor position.")
    ...
    candidates = []
    for r in trades:
        if not r.get("order_id") or str(r.get("dry_run", "")).lower() == "true":
            continue
        exp = r.get("expiration_date")
        if exp and date.fromisoformat(exp) < today:
            continue  # already past expiration -- swing_settle_trades.py's job, not ours
        candidates.append(r)

    if not candidates:
        log.info("No currently-open (non-dry-run, non-expired) swing positions to monitor.")
        return
