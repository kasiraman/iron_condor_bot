"""
Settles the swing (multi-week) iron condor(s) and records the realized outcome.

Companion to swing_iron_condor_bot.py / swing_monitor_and_exit.py. Unlike the 0DTE and
weekend settle scripts (which settle trades tied to a specific, known day), a swing
position's expiration date is whatever was actually chosen at entry (nearest listed
expiration to SWING_TARGET_DTE), so this script settles any trade whose expiration_date
has ALREADY PASSED (<=  today) and doesn't have an outcome recorded yet -- not an exact
date match, so it's safe to run daily without needing to know in advance which day the
position will actually expire.

  1. Reads logs/swing_trades.csv (entry log, written by swing_iron_condor_bot.py) for
     rows that were actually submitted (not --dry-run, have an order_id), whose
     expiration_date has passed, and don't yet have a matching row in
     logs/swing_trade_outcomes.csv.
  2. Looks up the real order via Alpaca to get its status and the actual filled net credit.
  3. Pulls SPY's official close price on the EXPIRATION date.
  4. Computes the settlement value of the 4-leg position at that close price and the
     ...
     closed early by swing_monitor_and_exit.py, in which case it uses the real
     entry_credit vs exit_debit instead (see get_early_close()).
  5. Appends one row per trade to logs/swing_trade_outcomes.csv.

Fees: same multi-date approach as weekend_settle_trades.py -- sums real fee activity from
BOTH the entry date and whichever date the position's fate was decided (the expiration
date if held to expiration, or the early-close date if closed early by
swing_monitor_and_exit.py). See get_fees_for_dates() below.
"""
...
load_dotenv()
log = get_logger("swing_settle_trades")
...
def get_early_close(order_id):
    """Checks logs/swing_closed_early.csv (written by swing_monitor_and_exit.py) for a
    completed early close of this order. Returns None if the position was held all the
...
def get_fees_for_date(trade_client, date_str, symbols):
    """See settle_trades.py's get_fees_for_date() docstring for the full rationale on
    why FEE activity is summed for the whole date rather than filtered by symbol."""
    try:
...
def get_fees_for_dates(trade_client, dates, symbols):
    """Sums fee activity across multiple (deduplicated) dates -- see
    weekend_settle_trades.py's get_fees_for_dates() for the same pattern; a swing hold
    can generate fee activity on the entry date and, separately, whichever later date the
    position's fate was decided (expiration or early close)."""
...
def settle_row(trade_client, stock_data_client, row):
    ...
    if early:
        ...
        if abs(early_entry_credit - fill_credit) > 0.01:
            notes.append(
                f"entry_credit recorded by swing_monitor_and_exit.py (${early_entry_credit:.2f}) differs "
        ...
        notes.append(
            f"Position was closed EARLY by swing_monitor_and_exit.py (trigger={early.get('trigger')}, "
    ...
    if fee_error:
        ...
        n_sharers = max(
            len([t for t in all_swing_trades if t.get("date") in fee_dates or t.get("expiration_date") in fee_dates]),
            1,
        )
        fees = fee_total / n_sharers if n_sharers > 1 else fee_total
        if n_sharers > 1:
            notes.append(
                f"{n_sharers} swing trades share fee date(s) {fee_dates} -- Alpaca's fee activity for "
        ...
        if len(fills) > 4 and not early:
            notes.append(
                ...
                f"Alpaca order history, or whether swing_monitor_and_exit.py closed it. Fills: {fills}"
            )
...
def main():
    parser = argparse.ArgumentParser(description="Settle swing (multi-week) iron condor trades whose expiration has passed.")
    parser.add_argument(
        "--date", metavar="YYYY-MM-DD",
        help="Settle trades with this EXACT expiration date instead of all trades whose expiration has already passed.",
    )
    parser.add_argument(
        "--force", action="store_true",
        help="Re-settle rows even if already present in swing_trade_outcomes.csv, replacing the old row. "
    ...
    if args.force and not args.date:
        confirm = input(
            "--force without --date will re-settle EVERY previously-settled swing trade. Continue? [y/N] "
        )
    ...
    def is_due(row):
        if args.date:
            return row.get("expiration_date") == args.date
        exp = row.get("expiration_date")
        return bool(exp) and date.fromisoformat(exp) <= today

    candidates = [
        r for r in trades
        if r.get("order_id") and str(r.get("dry_run", "")).lower() != "true" and is_due(r)
    ]
